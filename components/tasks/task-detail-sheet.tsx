...
export function TaskDetailSheet({
  ...
  async function handleClone() {
    ...
    if (result.success && result.taskId) {
      ...
      router.push(`/projects/${task.project_id}/tasks/${result.taskId}`)
    } else {
      toast({ title: 'Error', description: result.error, variant: 'destructive' })
    }
  }
            ...
            <div className="flex flex-col h-full">
              ...
              <div className="flex items-center gap-2 border-b px-4 h-11 shrink-0 bg-background/95">
                ...
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-muted-foreground" onClick={() => router.push(`/projects/${task.project_id}/tasks/${task.id}`)}>
