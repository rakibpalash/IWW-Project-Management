...
import {
  CalendarIcon, Loader2, Plus, Search, Users, Lock, DollarSign,
  ChevronDown, Building2, CreditCard, GitBranch, Clock, FileText,
} from 'lucide-react'
...
function Section({
  ...
  icon: React.ReactNode; label: string; defaultOpen?: boolean; children: React.ReactNode
}) {
  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen}>
      <CollapsibleTrigger asChild>
        <button
          type="button"
          className="flex w-full items-center gap-2 rounded-lg border border-border bg-muted/40 px-4 py-2.5 text-sm font-medium hover:bg-muted/60 transition-colors"
        >
          {icon}
...
export function CreateProjectDialog({
  ...
  useEffect(() => {
    ...
    // Managers = super_admin + account_manager + project_manager
    ...
    // Staff with manager_id for hierarchy
    supabase.from('profiles')
      .select('id, full_name, email, avatar_url, role, is_temp_password, onboarding_completed, created_at, updated_at, manager_id')
  ...
  // Group staff by manager for hierarchy display
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      ...
      // Insert project manager as 'lead'
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-3">
            ...
            {/* ── Client & Billing ──────────────────────────────────────── */}
            <Section icon={<CreditCard className="h-4 w-4 text-muted-foreground" />} label="Client & Billing">
            ...
            {/* ── Hierarchy & Team ──────────────────────────────────────── */}
            <Section icon={<GitBranch className="h-4 w-4 text-muted-foreground" />} label="Hierarchy & Team" defaultOpen>
              {/* Project Manager */}
              <div>
                <label className="text-sm font-medium mb-1.5 block">Project Manager <span className="text-xs text-muted-foreground font-normal">(lead)</span></label>
              ...
              {/* Assign Staff with hierarchy grouping */}
                ...
                <div className="rounded-lg border border-border">
                  ...
                  {staffList.length === 0 ? (
                    ...
                    <ScrollArea className="h-44">
                      {showGrouped ? (
                        // Grouped by manager (hierarchy view)
                        <div className="p-1">
            ...
            {/* ── Timeline & Budget ─────────────────────────────────────── */}
            <Section icon={<Clock className="h-4 w-4 text-muted-foreground" />} label="Timeline & Budget" defaultOpen>
              <div className="grid grid-cols-2 gap-4">
            ...
            {/* ── Description ───────────────────────────────────────────── */}
            <Section icon={<FileText className="h-4 w-4 text-muted-foreground" />} label="Description">
              <FormField control={form.control} name="description" render={({ field }) => (
                <FormItem>
                  <FormControl>
                    <Textarea placeholder="Describe the project goals, scope, and any relevant details…" rows={4} {...field} value={field.value ?? ''} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )} />
